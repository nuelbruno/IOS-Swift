...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        //        NetworkActivityIndicatorManager.sharedManager.startDelay = 0.001
        //        NetworkActivityIndicatorManager.sharedManager.completionDelay = 2.0
    ...
    lazy var applicationDocumentsDirectory: NSURL = {
        // The directory the application uses to store the Core Data store file. This code uses a directory named "com.tacme.DMCA_Admin" in the application's documents Application Support directory.
    ...
    lazy var managedObjectModel: NSManagedObjectModel = {
        ...
        let modelURL = NSBundle.mainBundle().URLForResource("DMCA_Admin", withExtension: "momd")!
    ...
    func setLanguage(language:String) -> Void {
        ...
        loadMainStoryBoard(language.compare("en") == .OrderedSame ? "Main" : "Main_ar")
    }
    
    /**
     Get App Language
     
     - returns: language code - en for english and ar for arabic
     */
    func getAppLanguage() -> String {
        let defaults = NSUserDefaults.standardUserDefaults()
        if let language = defaults.stringForKey("AppLanguage") {
            return language
        }
        return "en"
    }
    ...
    func changeLanguage() -> Void {
        if getAppLanguage().compare("en") == .OrderedSame {
            setLanguage("ar")
        }else{
            setLanguage("en")
        }
    ...
    func isLoggedIn() -> Bool {
        return defaults.objectForKey("userinfo") != nil
    }
    
    /**
     load main storyboard
     
     - parameter storyboardName:	storyboard name to be launched
     */
    func loadMainStoryBoard(storyboardName:String) -> Void {
        self.window = UIWindow(frame: UIScreen.mainScreen().bounds)
        let storyboard = UIStoryboard(name: storyboardName, bundle: nil)
        if(isLoggedIn()){
            self.window?.rootViewController = storyboard.instantiateViewControllerWithIdentifier("AppNavigationVC") as! UINavigationController
        }else{
            self.window?.rootViewController = storyboard.instantiateViewControllerWithIdentifier("LoginVC") as! LoginVC
        }
    ...
    /**
     Set up the reachability
     
     - parameter hostName:					hostname
     - parameter useClosures:				usercolosure
     - parameter completionHandler:	completion handler
     */
    ...
    /**
     called on change of network status
     
     - parameter note:	notification package
     */
